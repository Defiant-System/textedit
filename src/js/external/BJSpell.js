 ...
 */
 ...
 let BJSpell = function() {
	...
	function BJSpell(dic, callback) {
		if (this instanceof BJSpell) {
			...
			if (BJSpell[lang]) {
				...
				window.fetch(`/cdn/dict/${dic}.js`, { responseType: "text" })
					.then(resp => {
						let omit = `BJSpell.${dic}=`,
						    dStr = resp.data.slice(omit.length);
						self[dic] = new Function("BJSpell", resp.data).call({}, self);

						// new Function("BJSpell", resp.data).call({}, BJSpell);

						// console.log( self[dic] );
					});
				// var script = document.createElement("script");
				// BJSpell[lang] = self;
				// script.src = `/cdn/dict/${dic}.js`;
	...
	BJSpell.prototype.parse = function(word) {
		if (/^[0-9]+$/.test(word)) return this.checked[word] = true;
